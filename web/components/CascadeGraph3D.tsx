...
function buildHopRings(maxHop: number): THREE.Group {
  ...
  for (let hop = 1; hop <= maxHop; hop++) {
    ...
    // Subtle dashed ring at the hop's plane.
    const geo = new THREE.RingGeometry(r - 1.2, r + 1.2, 128);
    const mat = new THREE.MeshBasicMaterial({
      color: 0x22d3ee,
      transparent: true,
      opacity: 0.18 + 0.05 * hop,
...
export function CascadeGraph3D() {
  ...
  useEffect(() => {
    ...
    scene.add(rings);
    // Subtle fog so far-hop nodes fade into the dark — sells the depth.
    const prevFog = scene.fog;
    scene.fog = new THREE.Fog(0x000000, HOP_RADIUS * 1.5, HOP_RADIUS * (data.maxHop + 1) * 1.8);
    return () => {
      scene.remove(rings);
      rings.traverse((obj) => {
  ...
  // When the reveal completes, slow-orbit the camera so judges see depth.
  useEffect(() => {
    if (!fgRef.current) return;
    ...
    const tick = () => {
      // Very slow orbit — quarter of the previous speed. The camera stays
      // high so the dome of each hop shell never collapses to a line.
      theta += 0.0005;
